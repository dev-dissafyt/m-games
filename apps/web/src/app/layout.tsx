  ...
  return (
    <html lang="en" className="dark scroll-smooth">
      ...
      <body className="min-h-screen flex flex-col bg-[#030305] text-zinc-100 selection:bg-neon-cyan selection:text-black antialiased arcade-grid-bg">
        ...
        {/* Mobile Navigation Modal (Rendered outside <header> to avoid CSS backdrop-filter containing block truncation) */}
        {mobileMenuOpen && (
          <div
            className="md:hidden fixed inset-0 z-50 bg-black/85 backdrop-blur-md overflow-y-auto overscroll-contain p-3 sm:p-6 flex flex-col items-center animate-in fade-in duration-150"
            onClick={() => setMobileMenuOpen(false)}
          >
            {/* Solid Elevated Modal Card */}
            <div
              className="relative w-full max-w-lg bg-[#0b0e17] border border-zinc-700 shadow-2xl rounded-3xl my-2 sm:my-6 shrink-0 overflow-hidden animate-in zoom-in-95 duration-200"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header Bar */}
              <div className="h-16 px-4 sm:px-5 flex items-center justify-between border-b border-zinc-800 bg-[#0e131f] shrink-0">
                <Link
                  href="/"
                  onClick={() => setMobileMenuOpen(false)}
              ...
              {/* Modal Content */}
              <div className="px-4 sm:px-5 py-4 pb-8 space-y-4">
